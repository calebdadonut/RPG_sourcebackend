 ...
 */
Mage::Mage(string characterName, Race characterRace) : Character::Character(characterName, characterRace)
{
	this->SetHealth(30);
	Weapon SS;
	SS.name = "Simple Staff";
	SS.cost = 100;
	SS.damage = 5;
	this->SetWeapon(SS);
	intelligence = 10;
...
void Mage::Attack(Character* target)
{
	...
	cout << "Mage attacks " << target << " with " << this->GetWeapon().name << " for " << damage << " points " << endl;
}
/**
...
{
	int damage = 10 + this->GetWeapon().damage + intelligence / 2;
	target->TakeDamage(damage);
	cout << "mage attacks " << target << " with a fireball for " << damage << " points " << endl;
}
/**
